...
def read_matfile(filename="ETH-80"):

    matfile_dir = "../data/%s/%s" % (filename, filename)
    if filename == 'UCF':
        f = loadmat(matfile_dir + "_d22.mat")
    elif filename == "YaleB":
        f = loadmat(matfile_dir + "_set_d32.mat")
        g = loadmat(matfile_dir + "_set_traintest.mat")
    elif filename == 'ETH-80_highres':
        f = loadmat("../data/ETH-80/ETH-80_quad_d30.mat")
        g = loadmat("../data/ETH-80/ETH-80_traintest.mat")
    elif filename == 'YaleB_highres':
        f = loadmat("../data/YaleB/YaleB_set_dubbled_d30.mat")
        g = loadmat("../data/YaleB/YaleB_set_traintest.mat")
    elif filename == 'UCF_highres':
        f = loadmat("../data/UCF/UCF_quad_d22.mat")
    else:
        f = loadmat(matfile_dir + "_d30.mat")
        g = loadmat(matfile_dir + "_traintest.mat")
    ...
    if 'data' in f.keys():
        ...
        data = f['dataset']
    labels = f['labels']
    if filename not in ['UCF', 'UCF_highres']:
        data_split = g['ids']
    else:
        data_split = None
...
if __name__ == '__main__':
    imgs, labels, ds = read_matfile(fname="ucf")
    print(imgs.shape)
    print(labels.shape)
    print(ds[0][0].shape)
    # load_ETH80()
